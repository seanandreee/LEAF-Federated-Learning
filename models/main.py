...
def main():
    ...
    tup = MAIN_PARAMS.get(args.dataset, {}).get(args.t, (6, 2, 2))
    ...
    # Create 2 models
    model_key = f'{args.dataset}.{args.model}'
    if model_key not in MODEL_PARAMS:
        logger.error('Model %s not found in MODEL_PARAMS', model_key)
        return
    ...
    logger.info("Creating client model")
    client_model = ClientModel(args.seed, *model_params)

    # Create server
    logger.info("Creating server model")
    ...
    # Close models
    logger.info("Closing server model")
    server.close_model()
